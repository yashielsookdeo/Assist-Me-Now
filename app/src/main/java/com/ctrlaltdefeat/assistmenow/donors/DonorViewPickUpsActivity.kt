...
class DonorViewPickUpsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private lateinit var requests: List<Models.FinalRequests>
    ...
    private fun requestDirectionToMarker(markerPosition: LatLng) {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            val directionsResult: DirectionsResult = DirectionsApi.newRequest(geoApiContext).origin("${origin.latitude},${origin.longitude}").destination("${destination.latitude},${destination.longitude}").mode(TravelMode.DRIVING).await()
    ...
    private fun processRequests() {
        for (request in requests) {
            ...
            for (item in request.request) {
                snippetText = snippetText + item.item + " | "
            }

            val marker = gMap.addMarker(
                MarkerOptions().position(LatLng(request.latitude, request.longitude))
                    .title(request.name + " | " + request.creator)
                    .snippet(snippetText)
                    .contentDescription("This is a description?")
            )

            if (marker != null) {
                marker.tag = request.uid

                gMap.setOnMarkerClickListener { clickedMarker ->
                    val markerID = clickedMarker.tag as? String
                    var markerRequestName = ""

                    for (tempRequest in requests) {
                        if (tempRequest.uid == markerID) {
                            markerRequestName = tempRequest.name
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_donor_view_pick_ups)

        initButtons()

        Firebase.getProcessedRequests() { success, requestsList ->
            if (success) {
                requests = requestsList

                initMap(savedInstanceState)
            } else {
                Toast.makeText(this, "Failed to get pick ups - please try again.", Toast.LENGTH_LONG).show()
            }
        }
    }
